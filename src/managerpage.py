...
class manager:
    def __init__(self,root,myuid,mypass):
        ...
        order_button = Button(self.frame2, command=self.orderui, text="Order", activebackground="cyan", font=(40))
    ...
    def assignworkui(self):
        ...
        self.assigntoo = Text(self.frame3, font=("times new roman", 12), bg="white")
        self.assigntoo.place(x=220, y=335,width=220,height=30)
    ...
    def assignwork(self):
        ...
        if self.workdesc.get(1.0, END) == "" or self.jobid.get() == "" or self.assigntoo.get(1.0, END) == "":
            ...
            try:
                ...
                if row != None:
                    ...
                    self.cur.execute(
                        ...
                        (self.jobid, self.workdesc.get(1.0, END), self.myuid, self.assigntoo.get(1.0, END), x_date))
    ...
    def inventoryviewui(self):
        ...
        root1.geometry("1070x450")
        columns = ('inventoryid', 'inventoryitems', 'inventorydesc', 'inventoryprice', 'inventorynumber','inventoryexpiry')
        viewtree = ttk.Treeview(root1, columns=columns, show='headings', height=30)
        viewtree.pack()
        ...
        try:
            self.cur.execute(
                "select inventoryid, inventoryitems, inventorydesc, inventoryprice, inventorynumber, inventoryexpiry from inventory".format(
                    self.myuid))
            row = self.cur.fetchall()
            if row == None:
    ...
    def orderviewui(self):
            ...
            root1.title("orders")
            root1.geometry("1070x450")
            columns = ('order_id', 'order_amount', 'order_type', 'order_desc', 'order_customer_id')
            viewtree = ttk.Treeview(root1, columns=columns, show='headings', height=30)
            viewtree.pack()
            viewtree.heading('order_id', text='Order Id')
            viewtree.heading('order_amount', text='Order Amount')
            viewtree.heading('order_type', text='Order Type')
            viewtree.heading('order_desc', text='Order Decription')
            viewtree.heading('order_customer_id', text='Order Customer Id')
            viewtree.column("order_id", width=100, anchor=CENTER)
            viewtree.column("order_amount", width=550, anchor=CENTER)
            viewtree.column('order_type', width=120, anchor=CENTER)
            viewtree.column('order_desc', width=120, anchor=CENTER)
            viewtree.column('order_customer_id', width=120, anchor=CENTER)
            viewtree.pack(pady=30)
            try:
                self.cur.execute(
                    "select order_id, order_amount, order_type, order_desc, order_customer_id from orders".format(
                        self.myuid))
                row = self.cur.fetchall()
                if row == None:
    ...
    def generateorderui(self):
        messagebox.showinfo(title="Generate Order", message="No Data File Found.")

    def cancelorderui(self):
        messagebox.showinfo(title="Cancel", message="No Data File Found.")
